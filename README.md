...
参考所提供的 Nodeseek Pro 脚本：用户资料请求串行执行，上一条完成后间隔 300ms，首次请求立即执行；翻页、签到等同站 fetch 请求不增加固定等待，并优先于排队中的资料请求；支持 Web Locks 的浏览器跨标签页共享队列锁。403、429、503 会暂停后续请求至少 60 秒，并遵守更长的 `Retry-After`。这是一组保守的客户端策略，不是 NodeSeek 官方公布的配额；联网检索未找到可确认的官方频率上限。`Retry-After` 的语义参见 [MDN](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Reference/Status/429)。

用户资料缓存 6 小时，最多保留 200 人。列表互动计数默认悬停后读取，不再因读取失败创建隐藏页面；需要自动预读时可在“原生列表增强”中开启。网站本身的请求、手动互动页面和其他插件不受此队列控制。
