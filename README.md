# NodeSeek++

面向 **NodeSeek / DeepFlood** 的模块化用户脚本，提供阅读增强、帖子互动、正则监控、图片上传和用户资料展示。功能可独立开关，桌面与移动端使用响应式面板。

## 下载与安装

| 入口 | 地址 |
| --- | --- |
| 安装 / 更新（推荐） | [点击安装 NodeSeek++](https://update.greasyfork.org/scripts/595488/NodeSeek%2B%2B.user.js) |
| Greasy Fork | [脚本介绍与版本记录](https://greasyfork.org/zh-CN/scripts/595488) |
| GitHub | [rirh/nodeseek-plus](https://github.com/rirh/nodeseek-plus) |
| GitHub 分发文件 | [完整 userscript](https://github.com/rirh/nodeseek-plus/blob/main/dist/nodeseek-plus-plus.user.js) |
| 问题反馈 | [GitHub Issues](https://github.com/rirh/nodeseek-plus/issues) |

1. 在浏览器中安装 [Tampermonkey](https://www.tampermonkey.net/) 等用户脚本管理器。
2. 打开上方“安装 / 更新”链接，在脚本管理器页面点击“安装”。
3. 打开或刷新 [NodeSeek](https://www.nodeseek.com/) / [DeepFlood](https://www.deepflood.com/)。
4. 点击右下角设置图标，或从脚本管理器菜单打开 **NodeSeek++ 设置**。

设置按「浏览、界面、用户、工具」分类，支持搜索、导入、导出和恢复默认。「关于」提供当前版本、GitHub、下载安装及问题反馈入口。常规配置点击「保存并刷新」生效。

如果链接显示源码而没有弹出安装页，可复制完整 `.user.js` 内容，在脚本管理器中新建脚本、粘贴并保存。`.meta.js` 只有版本信息，不能用于安装。

## 如何更新

**设置内更新：** 打开设置，点击顶部或「关于」中的“检查更新” → 发现新版后点击“前往更新” → 在脚本管理器中确认更新 → 刷新论坛页面。

**手动更新：** 重新打开 [安装 / 更新链接](https://update.greasyfork.org/scripts/595488/NodeSeek%2B%2B.user.js)，在脚本管理器中确认覆盖更新，再刷新论坛。无需卸载旧版，以免丢失脚本管理器中的配置。

正式版也会自动检查更新：页面打开约 30 秒后检查，之后每 6 小时检查一次；发现新版本通过弹窗提醒，同一版本 24 小时内不重复提醒。只有论坛页面保持打开时，脚本的后台检查才会运行。

GitHub 分发文件可能比油叉同步更早；普通用户建议使用上方油叉安装链接。维护者的 `make deploy` 发布流程见本文末尾。

## 主要功能

| 功能 | 说明 |
| --- | --- |
| 阅读增强 | 可暂停自动翻页、外链直达、阅读历史、已读标记、代码高亮、帖子预览、长文折叠 |
| 帖子互动 | 原生列表、按需互动计数、快捷回复模板、回帖足迹 |
| 用户资料 | 等级、加入天数、本地参与参考分及明细，支持柔和单色与自定义徽章颜色 |
| 内容过滤 | 标题关键词、用户名和权限帖过滤，支持隐藏、折叠或高亮 |
| 帖子监控 | RSS 标题正则匹配、未读结果、后台轮询、页面提示及可选系统通知 |
| 抽奖追踪 | 手动追踪帖子，检测标题中的开奖线索 |
| 图片上传 | 支持 NodeImage、Telegraph、Telegraph2、LskyPro、Chevereto、EasyImages |
| 消息中心 | @我、回复主题作为固定联系人，与系统消息和私信会话共用左侧列表；分类与系统消息使用固定图标头像，右侧页内查看通知或聊天 |

消息中心会将已拉取的私信会话和聊天记录存入当前浏览器的 IndexedDB，按站点与账号隔离。每次同步按消息 ID 去重合并，不因服务器只返回近期数据而删除本地旧记录，也不按天数自动过期；只能保存已经拉取过的内容。先显示本地记录，再同步服务器；长会话先显示最近 200 条，可继续加载更早的本地记录。

聊天输入区默认使用新版 Markdown 编辑器，支持加粗、斜体、删除线、标题、列表、引用、链接、代码块、表格、分隔线、撤销/重做、清除格式、预览与导入 `.md` 文件。图片上传复用设置中的图床协议，支持选图、粘贴、拖拽；图片处理完成前不能发送，切换会话后仍插入原会话草稿。新版挂载在站点原通知容器内，与原版互斥显示；可切换“原版编辑器”，再通过容器内的“切回新版聊天”返回，不会同时出现两套聊天或自动发送草稿。
| 界面整理 | 隐藏侧栏广告、紧凑只读与置顶标记、图标工具栏 |
| 操作辅助 | 手动签到、可选自动签到、回复快捷键及 AI 文本美化 |

图片上传、帖子监控与抽奖选项默认开启；已有配置中明确关闭的选项会保留。新配置默认开启自动签到；旧配置若关闭了自动签到，需在“签到”设置中开启。

## 请求与阅读设置

在“等级、信任分与身份徽章”中可单独关闭“用户资料悬浮预览”，不影响资料徽章。默认开启，桌面鼠标停留约 300ms 后展示；手机及窄屏保留站点原有点击行为。帖子内容预览可通过“帖子卡片预览”功能开关单独关闭。

后台页面暂停无效的界面扫描和动画，监控的新帖检查与系统通知仍继续运行；系统休眠、浏览器冻结标签页仍可能推迟检查。

“网络 → 接口请求并发与延迟”可设置同页站内请求的最大并发数（1–10，默认 4）和相邻请求启动间隔（0–5000ms，默认 200ms）。首次请求立即执行，后续请求需同时满足并发额度与启动间隔；翻页、签到等用户操作优先于排队中的资料请求。各标签页独立调度，403、429、503 会共享冷却状态，暂停后续请求至少 60 秒，并遵守更长的 `Retry-After`。关闭该设置后使用默认值。

用户资料按站点持久缓存 24 小时，刷新和翻页复用，不再因超过 200 人提前淘汰有效资料；页面内存缓存同样检查有效期，过期后按需刷新，写入时清理过期记录。同一用户的并行读取共用一次请求，失败结果不缓存。可见作者优先加载，屏外作者接近视口时加载，头像及最后回复者的隐藏资料在悬停、聚焦或点击时读取。列表互动计数默认悬停后读取，不再因读取失败创建隐藏页面；需要自动预读时可在“原生列表增强”中开启。网站本身的请求、手动互动页面和其他插件不受此队列控制。

“等级、信任分与身份徽章”默认保留原有彩色样式，可手动选择自定义或柔和单色；自定义分别调整等级与天数、信任分、身份颜色。评论自动翻页开启后，工具栏提供暂停／继续按钮；暂停后仍可手动加载一页。用户卡片仅绑定用户名和头像，卡片触发链接不再保留重复的原生 title 提示。

## 帖子监控

### 配置一次，主要查看结果

NodeSeek 监控使用 https://rss.nodeseek.com/，每轮读取一次订阅。需要允许脚本连接 `rss.nodeseek.com`；该订阅不覆盖 DeepFlood，因此 DeepFlood 页面不启用此监控模块。监控入口位于右侧工具栏第一位。主面板只展示正则匹配到的帖子和未读结果，点击「配置」打开独立弹窗：

- **监控正则**：每行一个表达式，任意一条命中RSS 标题即可收录，不读取摘要或正文。
- **刷新间隔**：60–3600 秒，默认 300 秒。
- **更新**：保存配置、清空旧结果与未读，立即发起一次检查，不等待下一轮自动刷新。

不带分隔符的表达式默认忽略大小写，也支持 `/表达式/标志` 格式。无效正则会提示错误，不会应用。

```text
vmiss|搬瓦工
/香港.*(年付|月付)/i
```

两行分别匹配包含 `vmiss` 或「搬瓦工」的内容，以及「香港」后出现「年付」或「月付」的内容。不同规则按顺序分色。

不会写正则时，可以把需求交给 AI，例如：

> 请生成 JavaScript 正则，匹配包含 vmiss 或搬瓦工、但不包含已出的帖子，忽略大小写。按 /表达式/i 格式输出，每行一个，不要代码块。

检查生成结果后再使用。输入框旁的帮助图标可打开 [MDN 正则表达式文档](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_expressions)。

### 刷新与提醒

- **刷新**立即触发一次手动检查，跳过常规轮询等待；正在执行时不会重复并发。
- **停止 / 启动**控制自动监控；停止后仍可手动刷新一次。底部每秒更新下次检查倒计时，暂停、检查中和冷却中显示对应状态。
- 更新、手动刷新、检查完成或失败均有 Toast 反馈，检查期间显示 GSAP 动画。
- 首次检查 RSS 当前返回的 20 条并保存最新帖子 ID，不把已有匹配帖子全部作为新帖提醒。之后只处理 ID 大于上次检查位置的新帖，刷新页面仍保留进度；请求失败或返回旧数据不会推进或回退位置。
- 后续发现的新匹配帖子去重提醒，最多保留 200 条未读。打开面板后清空未读标记，面板统一展示已收录的匹配帖子（最近 200 条），不再单列「新发现」。
- 面板显示已成功检查的轮数、累计去重检查的帖子总数、符合及不符合数量；更新配置后重置统计。
- 监控系统通知默认开启，优先使用油猴 `GM_notification`；发现新的匹配帖子时发出通知，点击可打开帖子。保留首次快照不提醒、后续新帖去重及多标签页协调；油猴通知不可用时尝试已授权的浏览器通知，失败会给出明确提示。操作系统需允许浏览器或脚本管理器显示通知。

后台标签页也会检查，但必须保持论坛页面打开。浏览器休眠、标签页冻结或系统休眠可能推迟轮询，脚本不提供独立于浏览器的后台服务。

RSS 检查保留多标签页协调、失败冷却与手动刷新。只匹配订阅标题，不匹配摘要和正文。订阅仅覆盖其当前返回的帖子，不保证捕获轮询间隙的所有帖子。手动抽奖追踪仍检查原帖，保留请求间隔与配额限制。

### 抽奖追踪

登录后可在帖子页面的监控面板中选择「追踪当前抽奖帖」。检测到标题中的「已开奖」「开奖结果」等文字时给出提示，需要自行打开帖子核实；脚本不会据此判断是否中奖。

## 图片上传与快捷回复

在设置中选择图床协议及地址，然后在编辑器旁输入凭据并选择图片。选择文件后会上传到所选图床，并插入返回的链接。NodeImage 固定使用官方 API，并通过脚本管理器跨域上传；安装或更新时需允许连接 `api.nodeimage.com`。脚本会通过 NodeImage 登录会话自动获取密钥；未登录时打开上传栏的官网入口，完成 NodeSeek 授权后返回论坛即可重试。支持选择多张图片、在编辑器粘贴或拖拽图片；NodeImage 使用自动登录，不再显示手动密钥框；点击编辑器原有图片按钮即可选择文件。其他图床须支持所需接口与 CORS；上传凭据仅保留在当前页面内存，刷新后需重新输入。

列表中分类标签后的“快速回复”按钮打开快捷回复面板，支持搜索、新增、修改和删除模板。**点击模板内容会直接发送回复**，需要站点登录和相关权限。

## 数据与权限

- 常规配置、监控结果、历史和回复模板保存在用户脚本管理器的本地存储中；部分数据按站点及账号隔离。
- 设置导出不包含运行历史，并排除约定的密钥字段。监控正则独立保存，不随常规设置导出。
- 用户资料参考分根据公开注册时间、发帖数和评论数在本地计算，非站点官方评分，不代表交易信用。
- 图片上传会向所选图床发送图片；AI 美化会在手动操作时向所配置服务发送编辑器文本。
- 网站接口、登录状态及站点验证仍会影响功能。脚本不会绕过这些限制。

## 本地开发

使用支持当前 Vite 与 TypeScript 工具链的 Node.js 环境，并安装 pnpm。

```bash
pnpm install
pnpm dev
```

按照开发服务器输出的地址安装开发脚本，在目标论坛页面调试。

```bash
pnpm typecheck
pnpm build
pnpm test
```

测试会读取构建产物，因此应先执行 `pnpm build`。本地测试通过不等于浏览器权限、站点验证和第三方图床已通过实站验证。

| 文件 / 目录 | 用途 |
| --- | --- |
| `src/features/` | 独立功能模块 |
| `src/settings/` | 设置面板 |
| `src/core/` | 配置、运行时与功能类型 |
| `src/views/` | 预览、互动与快捷回复视图 |
| `tests/` | 单元及构建产物测试 |
| `dist/nodeseek-plus-plus.user.js` | 正式安装脚本 |
| `dist/nodeseek-plus-plus.meta.js` | 更新检查用元信息 |
| `references/` | 上游参考代码与许可材料 |

隔离构建阶段版本：

```bash
pnpm build --mode stage
```

产物位于 `dist-stage/`，脚本名称为 `NodeSeek++ (Stage)`，不要作为正式版本上传。

## 发布到 Greasy Fork

1. 修改 `package.json` 的 `version`，保持脚本名称和 namespace 稳定。
2. 执行 `pnpm typecheck`、`pnpm build` 和 `pnpm test`。
3. 登录 [Greasy Fork](https://greasyfork.org/zh-CN)，首次发布创建脚本；后续更新在原脚本页面提交新版本。
4. 上传或粘贴 `dist/nodeseek-plus-plus.user.js` 的完整内容，填写中文介绍及变更说明。
5. 使用发布后的安装链接验证安装与升级。

构建配置使用非压缩输出，并把第三方说明与 highlight.js 许可证写入脚本。请保留 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 和 [references/highlight.js.LICENSE](references/highlight.js.LICENSE)，构建时会读取它们。

发布前查看 [油叉代码规则](https://greasyfork.org/zh-CN/help/code-rules) 和 [元信息说明](https://greasyfork.org/zh-CN/help/meta-keys)。

## 许可证与参考

项目声明使用 [GPL-3.0-only](LICENSE)。第三方依赖适用各自许可证，详见 [第三方说明](THIRD_PARTY_NOTICES.md)。

上游功能对照和参考来源见 [功能矩阵](docs/feature-matrix.md)；`references/upstream/` 用于保留参考材料，不作为运行时入口。


### 一键发布与油叉同步

设置顶部版本号旁提供“检查更新”。正式版会在页面打开约 30 秒后检查油叉版本，之后每 6 小时检查一次；后台失败不打扰，同一新版本 24 小时内只提醒一次。后台发现更新时会等页面可见且没有打开弹窗，再显示响应式更新提示；“前往更新”打开油叉安装页，“稍后提醒”保留当前版本。检查只读取 `.meta.js` 版本信息，不下载执行远程代码。开发版与 Stage 版不自动检查更新。

直接执行，无需提前手动提交源码：

```bash
make deploy
```

命令要求位于 `main` 且没有未解决的合并冲突，依次检查远端、类型检查、递增版本、构建、测试，将全部未忽略的源码修改（包括新增、删除文件）、版本及两个 dist 文件一起提交，然后推送 `origin/main`。不会强制推送或自动合并远端修改。同一分钟连续发布也会递增版本。

需要跳过类型检查和测试时使用 `make deploy SKIP_CHECKS=1`，仍会检查分支与远端并重新构建产物。默认 `make deploy` 保留检查。

只生成本地产物可使用 `make deploy-local`。发布中途失败会保留文件和提交；检查并处理当前状态后重试。若只是推送失败，可直接重试 `git push origin main`。

油叉需完成一次性绑定：

1. 在已发布脚本的管理/同步页面设置代码同步地址：
   `https://raw.githubusercontent.com/rirh/nodeseek-plus/main/dist/nodeseek-plus-plus.user.js`
2. 在油叉账户的 Webhook 同步页面取得专属接收地址及配置说明。
3. 在 GitHub 仓库 Settings → Webhooks 中按油叉说明添加接收地址，订阅 push 事件。
4. 发布后检查 GitHub Webhook 的最近投递，以及油叉脚本页面的版本号与同步错误。

Webhook 接收地址可能包含账户凭据，不要提交到仓库。`make deploy` 推送成功只代表 GitHub 已更新，油叉异步同步仍需检查；首次发布与同步绑定不会由该命令自动创建。
