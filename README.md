...
同站 fetch 请求串行执行，至少间隔 3 秒；支持 Web Locks 的浏览器跨标签页共享队列锁。403、429、503 会暂停后续请求至少 60 秒，并遵守更长的 `Retry-After`。这是一组保守的客户端策略，不是 NodeSeek 官方公布的配额；联网检索未找到可确认的官方频率上限。`Retry-After` 的语义参见 [MDN](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Reference/Status/429)。

用户资料缓存 6 小时，最多保留 200 人。列表互动计数默认悬停后读取，不再因读取失败创建隐藏页面；需要自动预读时可在“原生列表增强”中开启。网站本身的请求、手动互动页面和其他插件不受此队列控制。

在“等级、信任分与身份徽章”中选择柔和单色、自定义或原有彩色；自定义分别调整等级与天数、信任分、身份颜色。评论自动翻页开启后，工具栏提供暂停／继续按钮；暂停后仍可手动加载一页。用户卡片仅绑定用户名和头像，卡片触发链接不再保留重复的原生 title 提示。
